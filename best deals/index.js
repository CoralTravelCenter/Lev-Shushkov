// Данный скрипт актуален только для Sunmar. На Coral вшита новая улучшеная логика вместе с редизайном страницы.

/* СТИЛИ ТОЛЬКО ДЛЯ ТЕСТОВ */
const bestDeaslStylesCORAL = `<style>
.bestdealsboxwrapper {
	visibility: visible;
	opacity: 1;
	pointer-events: auto;
  ...
  .owl-nav-container-promotion {
	display: flex;
	align-items: center;
	justify-content: center;
  }
  
  .owl-dot-container-promotion {
  ...
  .best-deals__dropdown-menu {
	z-index: 102 !important;
	top: 50px !important;
  }
  
  .best-deals__title-wrapper {
	display: flex;
  ...
  .best-deals__tab:last-of-type {
	transform: translateX(-5%);
	z-index: 2;
  }
  .best-deals__tab:nth-of-type(3) {
	transform: translateX(-2.5%);
	z-index: 4;
  }
  .best-deals__tab:nth-of-type(2) {
	z-index: 5;
  }
  .best-deals__tab:first-of-type {
	z-index: 6;
  }
  @media screen and (max-width: 1006px) {
	.best-deals__tab {
  ...
  .best-deals__tab_single:last-of-type {
	transform: translateX(0);
  }
  
  .best-deals__tab_active {
	z-index: 10 !important;
  }
  
  .best-deals__tab-link {
	text-align: center;
	display: flex;
	...
	box-shadow: 8px -7px 14px -12px rgba(0, 0, 0, 0.25);
	justify-content: center;
	width: 105%;
	font-size: 20px;
	line-height: 24px;
  ...
  @media screen and (max-width: 1006px) {
	.best-deals__tab-link {
  	...
  	.best-deals__tab-link_single {
		width: 100%;
  	}
  
  .best-deals__tab-link_active {
	background: #fff;
	border-bottom: 3px solid #fff !important;
  }
  
  .best-deals__title {
	background: linear-gradient(180deg, #f89402 0%, #ffe531 100%);
  ...
  .image-plate {
	...
	min-width: 123px;
	height: 27px;
	background: linear-gradient(90deg, #fbb200 0%, #ffcc00 100%);
	z-index: 2;
  }
  .image-plate::before {
	content: "";
  ...
  .bestdealsboxwrapper .bestdeals-body .item.exclusive:before {
	top: 62px !important;
  }
  
  .bestdealsboxwrapper .bestdeals-body .item.exclusive:after {
	top: 45px !important;
  }
  ...
  .best-deals__card-image-wrapper {
	opacity: 1 !important;
	margin: 0;
  ...
  .best-deals__card-content {
	padding: 0 !important;
	height: auto !important;
	position: relative !important;
  }
  
  .best-deals__card-text-content {
	display: flex;
	flex-direction: column;
	align-items: center;
  ...
  .best-deals__card-flights-price {
	display: flex;
	justify-content: center;
	align-items: center;
	flex-direction: column;
...
const bestDeaslStylesSUNMAR = `<style>
...
if (location.origin === "https://www.coral.ru") {
	head.insertAdjacentHTML("afterbegin", bestDeaslStylesCORAL);
}
if (location.origin === "https://www.sunmar.ru") {
	head.insertAdjacentHTML("beforeend", bestDeaslStylesSUNMAR);
}

/* <!-- НАЧАЛО СКРИПТА ДЛЯ ИНСАЙДЕРА --> */
...
// Проверка региона страницы. Текст кнопки сабменю меняется для МСК и СПБ (только там точное совпадение по странам из конфига)
...
// Отслеживание изменения контента в виджете. При изменении (после того как убирается прелоадер) запускается скрипт стилизации
...
// Фикс отображения буллитов навигации. По дефолту они все кривые.
...
// Стилизация карточек отелей в виджете
function setNewStyles() {
	const bestDealsNavigation = bestDealsContainer.querySelector(".owl-navigation");
	const allCardPages = bestDealsContainer.querySelectorAll(".owl-item");
	bestDealsContainer.classList.add("best-deals__styling"); // прелоадер стилизации (не доделан и не функционирует)
	let topRow, bottomRow, currentSceneCards;

	// В Owl слайдере все слайды по дефолту разбиты на 2 строки и на Сцены. Для мобильной и десктопной версии по разному.
	// Скрипт поочередно проходит каждую "страницу слайдера"
	allCardPages.forEach((page) => {
		...
		// Внутри каждой "страницы" проходимся поочередно по каждой карточке отеля
		currentSceneCards.forEach((card) => {
			...
			// Иногда дефолтный контент не приходит полностью и в этот момент не обнаруживается именно priceContainer.
			// Если такое случается - работа скрипта останавливается.
			...
			// Цена - строка с тонной пробелов в рандомных местах. Отсекаем пробелы.
			...
			// Установка шильдика с названием направления из изначальной строки со страной и направлением
			let countryRu;
			...
			// Если название на шильдике слишком длинное - шильдик делается большего размера
...
const setHeaderStyles = () => {
	...
	tabs.forEach((tab) => {
		...
		// Активную табу сделать неактивной
	...
	// Если таба всего одна - добавляется собственный стиль
	if (tabs.length === 1) {
		tabs.forEach((tab) => {
	...
	// Если есть таба для сабменю, то запускается изменение ее внутренностей
	if (dropdownTab) {
		...
		// Для МСК и СПБ запуск перестановки табов
		if (currentPageCity === "Москва" || currentPageCity === "Санкт-Петербург") {
			tabs.forEach((tab) => {
		...
		/* Прячем страны в выпадающем списке, которые уже отражены в табах (только на десктопе) */
		window.innerWidth > 1006 ? hideDropdownCounties(allDropdownCountries, tabs, dropdownTab, dropdownMenu) : hideCustomCountry(allDropdownCountries);

		// Устанавливаем Турцию вперед
...
const hideDropdownCounties = (allDropdownCountries, tabs, dropdownTab, dropdownMenu) => {
	allDropdownCountries.forEach((country) => {
		...
		// Сравниваются названия стран в табах, при совпадении страна скрывается в сабменю
		tabs.forEach((tab) => {
		...
		// Если страна стала не видимой - ее можно удалить из разметки сабменю
...
// Скрытие кастомной страны (если она уже не нужна но в CMS все еще отображается)
const hideCustomCountry = (allDropdownCountries) => {
	allDropdownCountries.forEach((country) => {
		const countryDropdown = country.firstElementChild.getAttribute("data-name");
		if (location.origin === "https://www.coral.ru" && countryDropdown === coralTabs.hideTab) {
			country.remove();
		}
		if (location.origin === "https://www.sunmar.ru" && countryDropdown === sunmarTabs.hideTab) {
			country.remove();
		}
	});
};
...
const changeTabsOrder = (dropdownCountries, tabs, tabsList, tabsContainer, dropdownTab, dropdownMenu, allDropdownCountries, dropDownButton) => {
	...
	let firstTab, secondTab, thirdTab, fourthTab, fifthTab, moreTab;

	// Проход по всем странам в сабменю
	for (let i = 1; i < dropdownCountries.length; i++) {
		// если страны нет в сабменю - сохраняем ее параметры
		if (tabs[i] === undefined) {
			...
			// Рендер в соответствии с конфигом табов
			if (dataName === coralTabs.first || dataName === coralTabs.second || dataName === coralTabs.third || dataName === coralTabs.fourth || dataName === coralTabs.fifth) {
	...
	if (window.innerWidth > 1006) {
		...
		if (location.origin === "https://www.coral.ru") {
			...
			// инициализация данных для табов
			...
			// Если таба инициализирована - рендер
			...
			if (window.innerWidth > 1006) {
				// КОЛИЧЕСТВО ВКЛАДОК ПРИ КОТОРОМ УДАЛЯЕТСЯ ИЗ РАЗМЕТКИ ПОДМЕНЮ СО СТРАНАМИ И КНОПКА "ЕЩЕ" CORAL
...
// Перемещение Турции на первое место
const setTurkeyFirst = (tabs, tabsList, tabsContainer) => {
	tabs.forEach((tab) => {
		// Нашли Турцию и переместили вначало
		if (tab.firstElementChild.textContent === "Турция") {
			tabsList.insertAdjacentElement("afterbegin", tab);
		}
	});
	// Всем табам устанавливается обработчик клика
	tabs.forEach((tab) => {
		tab.addEventListener("click", function () {
			changeTabs(tab);
		});
		// А если это первая таба то инициализируется клик на нее
		if (tab.firstElementChild.textContent === "Турция" || tabs.length === 1) {
			tab.firstElementChild.click();
		}
	});
	// Если Турции нет, то актиивируется первая стоящая таба через клик
	if (!tabsContainer.querySelector(".best-deals__tab_active")) {
		tabs[0].firstElementChild.click();
	}
};

// Просто смена активных вкладок
...
// Инициализация цепочки скриптов
setNewTitle();
setNewStyles();
setHeaderStyles();

// Удаление из разметки ненужных контейнеров (на мобилке - десктопный, на десктопе - мобилыный)
